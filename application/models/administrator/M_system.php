...
class M_system extends CI_Model {
	...
	function group_set_acces($group_id) {
		...
		foreach ($_POST['sidebar_access_sidebar_id'] as $key => $var) {
			$data['sidebar_access_group_id'] = $group_id;
			$data['sidebar_access_sidebar_id'] = $var;
			$data['sidebar_access_read'] 	= (!empty(@$_POST['sidebar_access_read'][$var])) ? '1' : '0';
			$data['sidebar_access_create'] 	= (!empty(@$_POST['sidebar_access_create'][$var])) ? '1' : '0';
			$data['sidebar_access_update'] 	= (!empty(@$_POST['sidebar_access_update'][$var])) ? '1' : '0';
			$data['sidebar_access_delete']	= (!empty(@$_POST['sidebar_access_delete'][$var])) ? '1' : '0';
			$this->db->set($data)->insert($this->sidebar_access);
		}
		return $this->db->affected_rows();
	}
